...
class Discord4LetterChecker:
    ...
    async def send_progress_update(self):
        """Send progress update at 25%, 50%, 75% milestones"""
        total = 26 ** 4
        remaining = len(self.remaining)
        done = total - remaining
        pct = (done / total * 100)
        
        # Check which milestone we've passed
        for milestone in self.progress_milestones:
            if pct >= milestone and milestone not in self.progress_sent:
                self.progress_sent.add(milestone)
                ...
                embed = {
                    "title": f"📊 Progress Update: {milestone}% Complete",
                    "description": f"Checked {done:,} / {total:,} usernames",
                    "color": 0x00ff00,
                    "fields": [
                        {"name": "Progress", "value": f"{pct:.1f}%", "inline": True},
                        {"name": "Found Available", "value": f"{self.available:,}", "inline": True},
                        {"name": "RPS", "value": f"{rps:.1f}", "inline": True},
                        {"name": "Error Rate", "value": f"{error_rate:.1f}%", "inline": True},
                        {"name": "Remaining", "value": f"{remaining:,}", "inline": True},
                        {"name": "Time Elapsed", "value": f"{int(elapsed/60)} min", "inline": True}
                    ],
                    "timestamp": datetime.utcnow().isoformat()
                }
                payload = {"embeds": [embed], "username": "4-Letter Checker"}
                await self.webhook_queue.put((self.webhook_url, payload))
                break  # Only send one update per check
    ...
    async def stats_printer(self):
        while True:
            ...
            # Check and send progress updates at milestones
...
if __name__ == "__main__":
    ...
    if not cfg or not cfg.get("webhook"):
        print(f"{Fore.YELLOW}[!] No config found or webhook missing!")
        ...
        print(f"{Fore.CYAN}[*] Then run the script again\n")
        exit(0)

    # Auto-run with loaded config
