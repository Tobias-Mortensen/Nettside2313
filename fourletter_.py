...
class Discord4LetterChecker:
    def __init__(self, webhook_url, concurrency=150, use_proxies=False):
    ...
    async def check_username(self, session, username, proxy_idx=None):
        if self.use_proxies and proxy_idx is not None:
            ...
            wait = 0.15 - (now - last)  # Reduced from 0.3s - we have 10k proxies!
            if wait > 0:
        ...
        try:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    # Mark proxy as rate limited - will rotate naturally
                    # Short backoff - proxy rotation does the heavy lifting
