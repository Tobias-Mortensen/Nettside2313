...
def build_presence(status: str, custom_text: str = ""):
    ...
    game = {
        ...
        "id": "game",
        "assets": {
            "large_image": "mp:external/" + GAME_LARGE_IMAGE.replace("https://", "").replace("http://", ""),
            "large_text":  GAME_NAME,
            "small_image": "mp:external/" + GAME_SMALL_IMAGE.replace("https://", "").replace("http://", ""),
            "small_text":  GAME_NAME,
        },
        "buttons": [
            {"label": GAME_BUTTON_LABEL, "url": GAME_BUTTON_URL},
        ],
    }
    if GAME_START_TIME:
